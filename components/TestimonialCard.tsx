...
const TestimonialCard = ({ quote, author, role, company }: { quote: string; author: string; role: string; company: string }) => {
    return (
        <Card className="bg-gradient-to-tr from-indigo-950 via-indigo-200 to-indigo-800 border-none">
            <CardContent className="p-6 h-full flex flex-col justify-between ">
                ...
                <div className=''>
                    <p className="font-medium text-neutral-950/70">{author}</p>
                    <p className="text-sm font-medium text-neutral-950">{role}, {company}</p>
                </div>
            </CardContent>
        </Card>
    );
